...
remove(curDate, dayStepsSum, day, stepsByDay, meanStepsPerDay, medianStepsPerDay)
```
...
```{r echo = FALSE}
print(intervals[which(avgStepsByInterval == max(avgStepsByInterval), arr.ind = TRUE)])
```
...
```{r echo = FALSE}
library(ggplot2)
p <- ggplot(mydat, aes(interval, steps), facets = .~daytype, geom = "line")
p <- p + facet_wrap(~ daytype, nrow = 2, ncol = 1)
p <- p + geom_line()
p <- p + labs(title = "Average number of steps per interval factored by weekday or weekend day"
                    , x = "Interval"
                    , y = "Steps (n)")
p
remove(avgStepsByIntervalWeekend
       , avgStepsByIntervalWeekday
       , avgStepsByInterval
       , curDate
       , day
       , days
